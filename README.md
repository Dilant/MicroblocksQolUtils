...
- “录制时切面自动保存（保证视频连续性）”默认开启：完整录像录制中（含手动录制）切面结束后，
  ...
  **本房间任意用户槽有存档时，停用内部自动存读档，死亡恢复交给 SpeedrunTool**，包括当前选中空槽的情况。
  其他房间的手动档仍保留，但不阻止新面按原规则自动保存；显式手动读档仍由 SRT 处理。
  手动 SL 的等待、动画和按键规则保持原样，我们只分段录像、恢复有效存档的录像前缀并优先硬切。
  SRT 的保存/读档/清档等进度提示不参与游戏帧采集、不推进恢复门，等待时间也不进入录像。
  清空最后一个用户存档只解除禁用，仍等录制开始、切面或复活点变化等原有触发条件；不立即补存，也不复用旧内部档。
  内部保存冻结游戏并显示“保存中”动画，干净恢复画面被录制后才放行更新；不覆盖/切走用户槽位，
