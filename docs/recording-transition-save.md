...
- 每次自动保存创建独立命名的内部 SpeedrunTool **版本槽**，不占用十个用户 UI 槽，不覆盖用户存档或仍被引用的旧版本。
  每次操作后恢复用户原来选中的槽；必须先等待内部异步 pre-clone 完成，避免后台线程读错槽。
- 内部槽保留在 SRT 槽字典，参与场景和保存回调重初始化。清全部用户槽时暂时摘下内部槽，
  然后重新放回；清当前用户槽自然不会碰到它。
- 切面、录制开始、复活点变化时始终按正常条件保存，不受手动槽是否非空影响。
  手动读档本身不排队自动保存；清档不立即存档或读档，也不改变当前录像分支，清档等待从视频中排除。
- 手动档绑定不可变录像前缀、复活前缀、录制源身份和当时的自动恢复版本。读档成功后才一起切换这些上下文。
  自动档的深克隆和 pre-clone 都成功才发布新版本；失败不替换仍有效的旧版本，新复活点不借用旧复活点。
- 当前分支、每个复活点的最新版本、仍存在的手动槽是保留根；覆盖/清槽后回收不再被引用的历史版本。
  例如 a1 → M → X → a2 → S2，再读 M → Y → a2′ → S3：S2 始终对应 X/a2，S3 对应 Y/a2′。
  读 M 后清掉 M 则回退到 a1，而不是全局最新的 a2′。清掉 S2 后仍可死亡回到该分支的 a2。
- 素材是同一次录制的追加写文件，分支只存剪辑引用，不复制视频；录制中不回收失败分支的源文件。
  默认 `.working/<区域>` 内保存 MKV、SFX/BGM PCM 和音乐事件；游戏快照和索引在内存。
  停止录制后的素材清理仍由现有导出流程负责。跨录制/无录像前缀的手动档不复用已结束的源文件，
  而是清空不相关前缀并开始新段；不承诺跨录制或重启游戏恢复录像历史。
...
- 稳定帧排队后，在 `Engine.Update` 入口拦住游戏、实体、物理与输入更新，不只是停录像或事后删掉卡顿。
  先呈现精确保存姿态并封口，再显示保存提示并在主线程执行场景存档；仅 SRT 自带的 pre-clone 工作在其后台线程执行。
...
- SRT 进度界面额外调用的 Present 不参与采集或推进暂停阶段。支持 `BusyIndicator.Begin/Wait` 时
  在同一个阻塞更新内复用 SRT 的保存和 pre-clone 提示，不绘制自己的保存 HUD；旧版/不可用时保留原动画。
  仅回收过时内部版本的清理提示静默；手动操作的提示仍正常显示。
...
- 当前选中非 TAS 用户槽已保存且启用 SRT 死亡自动读档时直接让行，SRT 自己接管死亡、等待与 wipe。
  其他未选中的手动槽不会全局禁用自动恢复；手动 SL 只分段录像，不使用内部冻结门。
