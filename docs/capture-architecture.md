...
### 手动 SpeedrunTool 存档优先

- 检查全部非内部槽的 `savedLevel.Session`；任意槽有本章节、本房间存档时取消内部保存计划/恢复资格，SRT 空闲后释放旧内部档。
  其他房间的存档不阻止新面自动保存。`TransitionTo` 按目标房间检查，不能因 `Session.Level` 暂时仍是旧房间而丢失请求。
  未知存档元数据保守视作手动优先；不通过切槽查询，不覆盖用户槽。
  死亡读档及其等待/动画由 SRT 接管，不加速死亡 End，不改变其自动读档设置。当前选中空槽也不例外。
  清空最后一个用户档只解除禁用，不生成保存请求；仍按录制开始、切面、复活点变化的原有规则保存，
  不立即在清档位置补存，也不复活旧内部档。
- beforeSave/beforeLoad 仅关闭两路录像分支；save 保存录像前缀，load 恢复经过验证的前缀。
  不在手动 load 后排队自动存档，也不应用内部游戏冻结门。
...
- 内部槽已有自己的保存提示，跳过重复的 SRT 提示及其同步全分辨率 backbuffer 读回。
  没有进度提示实现的旧版 SRT 不安装此 hook；用户手动操作的提示和游戏时序保持原样。
